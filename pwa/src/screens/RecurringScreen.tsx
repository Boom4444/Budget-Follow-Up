  ...
  return (
    <div className="flex flex-col h-full"
      ...
      <div className="bg-white border-b border-gray-100 px-4 py-3 flex items-center justify-between flex-shrink-0">
        <h1 className="text-[22px] font-bold">Récurrentes</h1>
      ...
      <div className="flex-1 overflow-y-auto scroll-ios">
        ...
        {recurring.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 text-center px-8">
            ...
            <p className="text-[17px] font-semibold text-gray-700 mb-1">Aucun modèle</p>
            <p className="text-[14px] text-gray-400 mb-4">Créez des modèles pour vos charges fixes et récurrentes</p>
            <button onClick={openNew}
...
function RecurringRow({ r, settings, i, total, onEdit, onDelete }: {
  ...
  return (
    <div className={`flex items-center gap-3 px-4 py-3 ${i < total - 1 ? 'border-b border-gray-100' : ''}`}>
      ...
      <div className="text-right flex-shrink-0">
        <p className="text-[15px] font-semibold">{r.amount.toFixed(2).replace('.', ',')} {CURRENCIES.find(c => c.code === r.currency)?.symbol}</p>
        <p className="text-[11px] text-gray-400">{FREQ_SHORT[r.frequency]}</p>
      </div>
      <button onClick={onDelete} className="text-gray-200 text-xl ml-1 active:text-red-400">×</button>
    </div>
  )
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-gray-50"
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between flex-shrink-0">
        ...
        <span className="font-semibold">{editItem ? 'Modifier' : 'Nouveau modèle'}</span>
      ...
      <div className="flex-1 overflow-y-auto scroll-ios">
        ...
        <div className="card mx-4 flex items-center px-4 gap-3">
          <input type="text" inputMode="decimal" placeholder="0,00"
            ...
            className="flex-1 py-3 text-[22px] font-semibold outline-none" />
        ...
        <div className="card mx-4 px-4 py-3 flex items-center justify-between">
          ...
          <label className="relative inline-flex items-center cursor-pointer">
            ...
            <div className="w-11 h-6 bg-gray-200 rounded-full peer-checked:bg-red-500 peer-checked:after:translate-x-5
        ...
        <div className="card mx-4 overflow-hidden">
          {(['weekly','biweekly','monthly','quarterly','yearly'] as RecurrenceFrequency[]).map((f, i, arr) => (
            <button key={f} type="button" onClick={() => setFrequency(f)}
              ...
              <span className="text-[15px]">{FREQ_LABELS[f]}</span>
            ...
            <div className="card mx-4 overflow-hidden">
              {FIXED_CATEGORIES.map((c, i) => (
                <button key={c.id} type="button" onClick={() => { setCategory(c.id); setIsFixed(true); setShowCatPicker(false) }}
                  className={`w-full flex items-center gap-3 px-4 py-3 ${i < FIXED_CATEGORIES.length - 1 ? 'border-b border-gray-100' : ''}`}>
